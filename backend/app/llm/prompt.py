...
statementテンプレート（肯定文のみ）:
T3: 「【対象】では【行為】を必ず行う。」
T4: 「【対象】では【行為】を必ず行う。」
T6: 「【条件】の場合、【行為】を行い、その後【確認】を行う。」
T7: 「【操作】の前に【前提条件】を確認する必要がある。」
T10: 「【条件】がある場合、【対応行為】を行う。」
T11: 「【判断条件】に該当する場合、【対応】を行う必要がある。」

重要ルール:
1. statementは必ず肯定文のみ（「する」「行う」「確認する」など）
2. 禁止事項（「してはならない」など）が引用にある場合:
   - 「必ず【逆の行為】を行う」に変換
   - 例: 「二重書き込みをしてはならない」→「ファイル編集時は必ず一人で行う」
3. 否定形・禁止表現は一切使わない（してはならない、禁止、NG、ダメ、しない、ではない）

...
T3: 「【状況・条件】の場合、【主体】は【行為】を必ず行う。」
  例: 「作業開始時において、担当者は安全確認を行う。」
T4: 「【タイミング・場面】において、【主体】は【基本ルール】に従って【行為】を行う。」
  例: 「定期的に、スタッフはマニュアルに従って点検を行う。」

【中級】
T6: 「【条件】の場合、【主体】は【行為】を行い、その後【確認】を行う。」
  例: 「異常が検出された場合、担当者は対応を行い、その後結果を確認する。」
T7: 「【操作】の前に【主体】は【前提条件・状況】を確認する必要がある。」
  例: 「作業を開始する前に、担当者は必要な準備が整っていることを確認する必要がある。」
T8: 「【状況・条件】の場合、【主体】は【行為】を行う。その理由は【理由】である。」
  例: 「特定の条件が満たされた場合、担当者は対応を行う。その理由は、安全性を確保するためである。」
T9: 「【状況A】の場合は【主体】は【行為A】を行い、【状況B】の場合は【行為B】を行う。」
  例: 「通常時においては担当者は通常の手順を行うが、特別な状況においては特別な手順を行う。」

【上級】
T10: 「【条件・状況】がある場合、【主体】は【対応行為】を行う。」
  例: 「特定の条件が満たされた場合、担当者は対応手順を実行する。」
T11: 「【判断条件・状況】に該当する場合、【主体】は【対応】を行う必要がある。」
  例: 「複数の条件が満たされた状況に該当する場合、担当者は対応を行う必要がある。」
T12: 「【通常ケース・状況】では【主体】は【通常行為】を行うが、【例外条件・状況】の場合は【例外行為】を行う。」
  例: 「通常時においては担当者は通常の手順を行うが、例外条件においては特別な手順を行う。」
T13: 「【判断基準1・状況】かつ【判断基準2・状況】に該当する場合、【主体】は【対応】を行う必要がある。」
  例: 「条件Aが満たされた状況かつ条件Bが満たされた状況に該当する場合、担当者は対応を行う必要がある。」
...
2. 【必須】すべてのstatementには文脈（状況・条件・タイミング・場面）を含めること:
   ...
   - 状況: 作業開始時、緊急時、通常時、異常が検出された場合など（引用に記載されている具体的な状況を使用）
   - 条件: 特定の条件が満たされた場合、必要な準備が整った場合など（引用に記載されている具体的な条件を使用）
   - タイミング: 作業開始前、作業中、作業終了後、定期的になど（引用に記載されている具体的なタイミングを使用）
   - 場面: 作業現場、特定の場所など（引用に記載されている具体的な場面を使用）
3. 禁止事項（「してはならない」「やってはいけない」「禁止」など）が引用にある場合:
   - 【重要】禁止されている行為をstatementに含めないこと
   - 禁止事項が含まれている引用の場合は、その引用から別の内容（許可されている行為、推奨されている行為など）を選択してstatementを生成する
   - 禁止事項を肯定文に変換してはいけない（例: 「やってはいけない」→「行う」は誤り）
   - 例: 引用が「店外での身体接触をしてはいけない」の場合、statementは「万引きに対処する場合、店舗スタッフは警察に通報する。」のように、禁止されていない別の行為を記述する
...
【絶対禁止】引用に含まれていないキーワードや内容をstatementに追加しないでください。引用に記載されている内容のみを使用してください。

【最重要】引用の内容をそのまま使用すること:
- 引用に「開店前」と書かれていれば「開店前」を使用（「作業開始時」などの汎用的な表現は使わない）
- 引用に「店舗スタッフ」と書かれていれば「店舗スタッフ」を使用（「担当者」などの汎用的な表現は使わない）
- 引用に「防犯カメラの録画ランプを確認する」と書かれていれば「防犯カメラの録画ランプを確認する」を使用（「安全確認を行う」などの汎用的な表現は使わない）
- 引用に記載されている具体的な状況・条件・主体・行為・手順をそのまま使用すること

理解度を深めるため、単純な事実確認ではなく、理由・方法・判断基準を含む内容を優先する。
ただし、禁止事項（「やってはいけない」「禁止」など）が含まれている引用の場合は、その禁止事項をstatementに含めず、引用内の別の内容（許可されている行為、推奨されている行為など）を選択してください。

【文脈を含める例】
❌ 悪い例: 「担当者は対応を行う。」（文脈が不明確：いつ？どの状況で？）
✅ 良い例: 「引用に記載されている具体的なタイミングにおいて、引用に記載されている主体は引用に記載されている行為を行う。」（状況と主体が明確）
✅ 良い例: 「引用に記載されている具体的な状況が発生した場合、引用に記載されている主体は引用に記載されている手順を実行し、その後引用に記載されている確認を行う。」（状況・行為・確認が明確）

【重要】上記の例はテンプレート形式です。実際のstatementでは「引用に記載されている」という表現は使わず、引用から抽出した具体的な内容を使用してください。
...
"""
    ...
    if len(citations_for_llm) == 0:
        ...
        context_parts = ["【引用】"]
        context_parts.append("以下の引用に基づいてstatementを生成してください。引用に記載されている具体的な内容（状況・条件・主体・行為・手順など）をそのまま使用してください。")
...
- 【最重要】引用に記載されている具体的な内容（状況・条件・主体・行為・手順など）をそのまま使用すること
- 【禁止】汎用的な表現（「作業開始時」「担当者」「安全確認」など）は使わず、引用に記載されている具体的な表現を使用すること
- 曖昧表現禁止（場合がある、望ましい等）
...
- 「引用に記載されている具体的なタイミングにおいて、引用に記載されている主体は引用に記載されている行為を行う」→ 文脈が明確（タイミング、主体が明確）
- 「引用に記載されている具体的な状況が発生した場合、引用に記載されている主体は引用に記載されている手順を実行し、その後引用に記載されている確認を行う」→ より具体的な文脈（状況・行為・確認が明確）

【重要】上記の例はテンプレート形式です。実際のstatementでは「引用に記載されている」という表現は使わず、引用から抽出した具体的な内容を使用してください。

【重要】禁止事項が含まれている引用の処理:
- 引用に「してはならない」「やってはいけない」「禁止」などの禁止表現が含まれている場合、その禁止されている行為をstatementに含めないこと
- 禁止事項を肯定文に変換してはいけない（例: 「やってはいけない」→「行う」は誤り）
- 引用内の別の内容（許可されている行為、推奨されている行為、手順など）を選択してstatementを生成する
- 例: 引用が「店外での身体接触をしてはいけない。警察に通報する。」の場合、statementは「万引きに対処する場合、店舗スタッフは警察に通報する。」のように、禁止されていない別の行為を記述する
...
【重要】上記のstatementと同一または類似の内容を出力しないでください。引用の内容に基づいて、異なるstatementを生成してください。
"""
